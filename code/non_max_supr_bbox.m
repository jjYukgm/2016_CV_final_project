function [is_valid_bbox , Nbboxes] = non_max_supr_bbox(bboxes, confidences, img_size, verbose)
    ...
    for j = find(is_valid_bbox)
        ...
        if iw>0 && ih>0                
            ...
            ua=(cur_bb(3)-cur_bb(1)+1)*(cur_bb(4)-cur_bb(2)+1)+...
               ...
               iw*ih;
            ov=iw*ih/ua;
            if ov > 0.25 %If the less confident detection overlaps too much with the previous detection
        ...
        %%%  take only one bbox if there exist two of them on the same column
%         if iw>0
%             ul=(cur_bb(3)-cur_bb(1)+1)+...
%                (prev_bb(3)-prev_bb(1)+1)-...
%                iw;
%             ov=iw/ul;
